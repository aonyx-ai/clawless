    ...
    pub fn inventory(&self) -> TokenStream {
        ...
        quote! {
            struct #inventory_name {
                ...
                func: fn(clawless::clap::ArgMatches, clawless::context::Context, clawless::cancellation::Cancellation) -> std::pin::Pin<Box<dyn std::future::Future<Output = clawless::CommandResult>>>,
            }
    ...
    pub fn submit_command(&self) -> TokenStream {
        ...
        quote! {
            clawless::inventory::submit!(super::#inventory_name {
                name: #command,
                init: #init_fn_name,
                func: |args, context, cancellation| Box::pin(#wrapper_fn_name(args, context, cancellation)),
            });
        }
    }
}
